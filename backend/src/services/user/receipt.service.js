...
//--------------------- VIEW RECEIPT SERVICE ---------------------
export const viewReceiptService = async (paymentId, hostUrl) => {
  ...
  const formattedAppointmentDate = payment.appointment?.appointmentDate
    ? new Date(payment.appointment.appointmentDate).toLocaleDateString("en-IN")
    : "-";
...
<style>
  * { box-sizing: border-box; }
  ...
  .invoice-card { max-width:820px; margin:auto; background:#fff; border-radius:18px; overflow:hidden; box-shadow:0 25px 60px rgba(0,0,0,0.08); }
  .top-bar { height:8px; background:linear-gradient(90deg, #4f46e5, #7c3aed); }
  ...
  .invoice-meta h2 { margin:0 0 10px 0; font-size:18px; font-weight:600; }
  .section-title { font-size:13px; font-weight:600; text-transform:uppercase; letter-spacing:1px; color:#64748b; margin-bottom:15px; }
  .info-grid { display:grid; grid-template-columns:1fr 1fr; gap:30px; margin-bottom:35px; }
  .info-card { background:#f9fafb; padding:18px; border-radius:12px; font-size:14px; line-height:1.8; }
  table { width:100%; border-collapse:collapse; margin-top:10px; }
  thead { background:#f1f5f9; }
  th { text-align:left; padding:14px; font-size:13px; font-weight:600; color:#475569; }
  td { padding:16px 14px; font-size:14px; border-bottom:1px solid #f1f5f9; }
  .amount { text-align:right; }
  .total-row td { font-weight:700; font-size:16px; border-top:2px solid #e2e8f0; }
  .status { margin-top:20px; }
  .status-badge { display:inline-block; padding:6px 16px; border-radius:50px; font-size:12px; font-weight:600; background:#ecfdf5; color:#059669; }
  ...
  <div class="invoice-card">
    ...
    <div class="content">
      ...
      <table>
        ...
        <tbody>
          <tr><td>${serviceType}</td><td class="amount">${(payment.amount/100).toLocaleString("en-IN",{minimumFractionDigits:2})}</td></tr>
          <tr class="total-row"><td>Total</td><td class="amount">${(payment.amount/100).toLocaleString("en-IN",{minimumFractionDigits:2})}</td></tr>
        </tbody>
      </table>

      <div class="status"><span class="status-badge">${payment.status?.toUpperCase()}</span></div>
      <div class="footer">This is a system-generated receipt.<br/>© ${new Date().getFullYear()} Pulse360</div>
