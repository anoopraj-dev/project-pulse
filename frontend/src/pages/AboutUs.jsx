...
const OfferCard = ({ icon, title, desc, featured }) => (
  <motion.div
    ...
    style={
      featured
        ? {
            background: "linear-gradient(150deg,#003554,#0077a8)",
  ...
  >
    ...
    <div className="flex-1 space-y-1.5">
      <h3
        className={`font-bold text-[1rem] ${featured ? "text-white" : "text-slate-900"}`}
      >
        {title}
      </h3>
      <p
        className={`text-sm leading-relaxed ${featured ? "text-white/55" : "text-slate-500"}`}
      >
        {desc}
      </p>
    </div>
  ...
  >
    <div className="relative w-20 h-20 mx-auto mb-4">
      <div
        ...
        style={{
          backgroundImage: `url(${doctor.profilePicture || "/images/doctor-avatar.jpg"})`,
          border: "3px solid #ddf1f8",
        }}
      />
    ...
    <motion.button
      ...
      onMouseLeave={(e) => {
        e.currentTarget.style.background = "#ddf1f8";
        e.currentTarget.style.color = "#0096C7";
      }}
      onClick={() => navigate("/signup")}
    >
...
const Testimonial = ({ name, rating, text }) => (
  ...
  >
    <div
      className="font-[Georgia,serif] text-5xl leading-none font-bold"
      style={{ color: "#0096C7" }}
    >
      "
    </div>
